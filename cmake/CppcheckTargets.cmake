...
function(add_cppcheck _name)
	...
	if(CPPCHECK_FOUND)
		...
		foreach(_include ${_cppcheck_includes})
            if (DEFINED CPPCHECK_EXCLUDE_DIRECTORIES)
                list (FIND CPPCHECK_EXCLUDE_DIRECTORIES ${_include} _index)
                if (${_index} GREATER -1)
                        continue()
                endif()
            endif()
		...
		foreach(_source ${_cppcheck_sources})
                        get_filename_component(_cppcheck_filename "${_source}" NAME)
                        if (DEFINED CPPCHECK_EXCLUDE_FILES)
                        list (FIND CPPCHECK_EXCLUDE_FILES ${_cppcheck_filename} _index)
                        if (${_index} GREATER -1)
                                continue()
                        endif()
                        endif()
		...
		add_custom_target(${_name}_cppcheck
			COMMAND
			${CPPCHECK_EXECUTABLE}
                        ${CPPCHECK_XML_OUTPUT_ARG}
                        ${CPPCHECK_XML_VERSION_ARG}
			${CPPCHECK_QUIET_ARG}
			${CPPCHECK_TEMPLATE_ARG}
			${_cppcheck_args}
			${_includes}
