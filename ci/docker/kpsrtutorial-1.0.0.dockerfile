...
RUN git clone https://github.com/klepsydra-technologies/kpsr-core.git && \
    ...
    cmake -DCMAKE_BUILD_TYPE=Release -DKPSR_WITH_DOXYGEN=true -DKPSR_WITH_DDS=false -DKPSR_WITH_ZMQ=false -DKPSR_TEST_PERFORMANCE=true -DKPSR_WITH_SOCKET=true -DKPSR_WITH_YAML=true -DKPSR_WITH_CODE_METRICS=true ../ && \
    make -j$(nproc) && \
    sudo make install && \
    cd ../../ && \
    rm -rf kpsr-core
...
ARG KPSRTUTORIALVERSION="kce-74"
